        ...
        <aside className="w-64 shrink-0 hidden md:flex flex-col gap-5 p-4 overflow-y-auto overflow-x-hidden" style={{ background: SIDEBAR, borderRight: "1px solid var(--border)" }}>
          ...
          <nav className="flex flex-col gap-1">
            {NAV.map((n) => { const on = n.id === "team"; const Icon = n.icon; return (
              n.href ? (
                <Link href={n.href} key={n.id} className="tm-nav flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-left" style={on ? { background: ACCENT_GRAD, color: ON_ACCENT } : { color: "var(--text-2)" }}>
                  <Icon size={18} /> {n.name}
                </Link>
              ) : (
                <button key={n.id} type="button" className="tm-nav flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-left" style={{ color: "var(--text-2)", opacity: 0.55, cursor: "default" }}>
                  <Icon size={18} /> {n.name}
                </button>
              )
            ); })}
