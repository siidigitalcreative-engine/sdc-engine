...
// ---------------------------------------------------------------------------
// Bot chat helpers (added for the SDC Assistant reply-in-chat bot).
// Reuse the same token / app_token / table config as everything above.
// ---------------------------------------------------------------------------

// Normalize any Lark Base field value into plain text.
function fieldText(v) {
  if (v == null) return "";
  if (Array.isArray(v)) return v.map(fieldText).filter(Boolean).join(", ");
  if (typeof v === "object") return String(v.text ?? v.name ?? v.en_name ?? "");
  return String(v);
}

// Format a Lark date field (epoch ms) as YYYY-MM-DD.
function fieldDate(v) {
  if (v == null || v === "") return "";
  const ms = Number(v);
  if (!Number.isFinite(ms)) return fieldText(v);
  return new Date(ms).toISOString().slice(0, 10);
}

// Read up to `limit` task records, shaped for a chat reply.
export async function listTasks(limit = 15) {
  const { token, url } = await base();
  const res = await fetch(`${url}?page_size=${limit}`, {
    headers: { Authorization: `Bearer ${token}` },
  });
  const data = await res.json().catch(() => ({}));
  if (data.code !== 0) throw new Error(`Base read failed (${data.msg || JSON.stringify(data)})`);
  const items = data?.data?.items || [];
  return items.map((rec) => {
    const f = rec.fields || {};
    return {
      title: fieldText(f[F_TITLE]),
      status: fieldText(f[F_STATUS]),
      date: fieldDate(f[F_DATE]),
      assignees: fieldText(f[F_ASSIGNEES]),
      desc: fieldText(f[F_DESC]),
    };
  });
}

// Reply to a specific incoming message with plain text.
export async function sendReply(messageId, text) {
  const token = await getToken();
  const res = await fetch(`${LARK}/open-apis/im/v1/messages/${messageId}/reply`, {
    method: "POST",
    headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
    body: JSON.stringify({ content: JSON.stringify({ text }), msg_type: "text" }),
  });
  const data = await res.json().catch(() => ({}));
  if (data.code !== 0) throw new Error(`Reply failed (${data.msg || JSON.stringify(data)})`);
  return true;
}
